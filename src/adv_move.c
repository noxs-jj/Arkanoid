...
static int	adv_move4(t_data *d, int side)
{
	if (d->ball.dir == 5)
...
static int	adv_move3(t_data *d, int side)
{
...
static int	adv_move2(t_data *d, int side)
{
	if (d->ball.dir == 1)
		do_dmg(d, 7, d->ball.posx + 1, d->ball.posy - 1);
...
static int	adv_move1(t_data *d, int side)
{
	if (d->ball.dir == 0)
...
int			adv_move(t_data *d, int side)
{
	...
	else if (side == 1 && d->ball.dir == 1)
	{
		...
		return (adv_move2(d, side));
	else if (side == 3 || side == 5)
		return (-1);
	else if (side == 4)
		return (adv_move3(d, side));
	else if (side == 6)
		return (adv_move4(d, side));
